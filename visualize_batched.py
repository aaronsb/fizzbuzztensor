...
def plot_3d_tensor_structure(save_path="docs/images/fizzbuzz_3d_structure.png"):
    ...
    ax1.set_title(
        "3D Tensor Structure\n(batch × sequence × divisors)", fontweight="bold", fontsize=12
    )
    ...
    im3 = ax3.imshow(
        div_tensor[:, :, 0], aspect="auto", cmap="Reds", interpolation="nearest", vmin=0, vmax=1
    )
    ...
    im4 = ax4.imshow(
        div_tensor[:, :, 1], aspect="auto", cmap="Blues", interpolation="nearest", vmin=0, vmax=1
    )
    ...
    plt.suptitle(
        f"Batched 3D Tensor: Shape {div_tensor.shape}", fontsize=14, fontweight="bold", y=0.98
    )
...
def plot_parallel_computation(save_path="docs/images/fizzbuzz_parallel.png"):
    ...
    for idx, ax in enumerate(axes):
        ...
        ax.bar(positions, np.ones(seq_len), color=bar_colors, edgecolor="black", linewidth=0.5)
        ...
        end_num = nums[idx, -1]
        ax.set_ylabel(f"Batch {idx}\n({start_num}-{end_num})", fontsize=11, fontweight="bold")
        ax.set_ylim(0, 1.2)
