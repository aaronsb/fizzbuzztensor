...
def plot_compact_matrix(save_path="docs/images/fizzbuzz_compact.png"):
    ...
    im = ax1.imshow(PATTERN_COMPACT, cmap="viridis", vmin=0, vmax=3, interpolation="nearest")
    ...
    bars = ax2.bar(
        representations, elements, color=colors, alpha=0.7, edgecolor="black", linewidth=2
    )
...
def plot_decision_tree(save_path="docs/images/fizzbuzz_decision_tree.png"):
    ...
    ax.set_title(
        "Binary Decision Tree for Compact FizzBuzz", fontsize=16, fontweight="bold", pad=20
    )
    ...
    ax.text(
        0.5,
        0.1,
        "Matrix Representation: PATTERN[div_by_3][div_by_5]",
        ha="center",
        fontsize=12,
        style="italic",
